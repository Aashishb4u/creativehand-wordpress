class VamtamForm extends elementorModules.frontend.handlers.Base {
	...
	onInit( ...args ) {
		...
		this.telValidationPatternFix();
	}

	telValidationPatternFix() {
		if ( ! jQuery( 'body' ).hasClass( 'e--ua-webkit' ) ) {
			return; // Only required on webkit.
		}

		const telInputs = jQuery( this.$element ).find( 'input[type=tel]' );
		jQuery.each( telInputs, function ( i, el ) {
			const $el = jQuery( el );
